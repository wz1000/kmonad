...
-- | A flag to enable maximum verbosity
flagVerbose :: Flag LogCfg
flagVerbose = mkFlag "verbose" logLevel LevelDebug
  "Set verbosity to maximum: same as log-level debug"

-- | An option to set the log-level
optLogLevel :: Option LogCfg
optLogLevel = mkOption "log-level" logLevel logLevelArg
  "Logging verbosity: 'debug' > 'info' > 'warn' > 'error'"

-- | An option to set logging-section separators
optLogSection :: Option LogCfg
optLogSection =  mkOption "log-sections" useSep onOffArg
  "Whether to use section separators in the logging output"

-- | An option to set logging-color
optLogColor :: Option LogCfg
optLogColor = mkOption "log-color" logColor logColorArg
  "What kind of coloring to use, for 'dark-bg', 'light-bg', or 'none'"

-- | Collection of all flags that will run on anything that has a LogCfg
logFlags :: HasLogCfg s => Named (AnyFlag s)
logFlags = collectFlags logCfg [flagVerbose]

-- | Collection of all options that will run on anything that has a LogCfg
logOpts :: HasLogCfg s => Named (AnyOption s)
logOpts = collectOpts logCfg [optLogLevel, optLogSection, optLogColor]
