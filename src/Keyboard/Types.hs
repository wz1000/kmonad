...
class HasKeyname a where keyname :: Lens' a Keyname

instance HasKeyname Keyname where keyname = id
...
{- SECTION: More keynames -----------------------------------------------------}

data KeyTable = KeyTable [(Keyname, Keycode)]
  deriving (Eq, Show)

class HasKeyTable a where keyTable :: Lens' a KeyTable

instance HasKeyTable KeyTable where keyTable = id

instance Default KeyTable where def = KeyTable []

codeForName :: HasKeyTable env => Keyname -> Getter env (Maybe Keycode)
codeForName = undefined

keyDict :: HasKeyTable env => Getter env (M.HashMap Keyname Keycode)
keyDict = undefined

withKeyTable :: a -> (KeyTable -> b) -> b
withKeyTable = undefined
